...
export async function reportCategory(
  ...
  const grouped = formatted.reduce(
    (acc, curr) => {
      const categoryId = curr.ticket.category.id;

      if (!acc[categoryId]) {
        acc[categoryId] = {
          categoryId: curr.ticket.category.id,
          categoryTitle: curr.ticket.category.title,
          total: 0,
          requests: [],
        };
      }
      ...
      acc[categoryId].requests.push(curr);
